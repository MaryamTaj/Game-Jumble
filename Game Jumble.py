# Author: Maryam Taj
# Name of Program: Game Menu
# Description of Program: This program allows the user to play multiple games developed throughout the ICS3U1 - 02 course.
...
def Yahtzee():
    ...
    # their total sum for 2's is 10.
        ...
        def smode(dice):
            if '1' not in pL or '2' not in pL or '3' not in pL or '4' not in pL or '5' not in pL or '6' not in pL :
                Sum,Bonus = 0,0
            else:
                Sum = sum(mode(dice))
                if Sum>=63:
    ...
    # Computers asks user to lock in a value of their choice. 
            ...
            if User not in pL:
                if User == 'Sm' or User == 'Bn':
                    if '1' not in pL or '2' not in pL or '3' not in pL or '4' not in pL or '5' not in pL or '6' not in pL:print("Lock in your 1,2,3,4,5 and 6's first.")
                    else:
                        pL[User] = tL[User]
                        break
                else:
